    ...
    if (isalpha_string(s)) // should return true for empty string too
    {
        ...
        if (v.size() < (uint32_t) 4) {
            ...
            cerr << "resulted in the wrong number of intervals. Perhaps ordering of numbers in GFA is irregular?!"
                 << std::endl;
            cerr << "Size of partition based on site " << next_site << " is " << v.size() << std::endl;
            exit(-1);
        }
        next_site += 2;
...
bool LocalPRG::vcf_ref_is_good(const std::string &vcf_ref){
    if (vcf_ref.length() < 30){
        cout << "Input vcf_ref path was too short to be the ref for PRG " << name << endl;
        return false;
    }
    ...
    if (refpath.empty()){
        cout << "Input vcf_ref was not a path in PRG " << name << endl;
        return false;
    }

    if (refpath[0]->pos.length > 0 or refpath.back()->pos.length > 0){
        cout << "Input vcf_ref path did not start/end at the beginning/end of PRG " << name << endl;
        return false;
    }
...
std::vector<KmerNodePtr> LocalPRG::find_path_and_variants(PanNodePtr pnode,
    ...
    //std::cout << "called find path and variants" << std::endl;
    std::string new_name = name;
    std::replace(new_name.begin(), new_name.end(), ' ', '_');

    std::vector<KmerNodePtr> kmp;
    kmp.reserve(800);
    std::vector<LocalNodePtr> lmp, almp, refpath;
    lmp.reserve(100);
    almp.reserve(100);
    refpath.reserve(100);
    float ppath;
    ...
    if (bin)
        ...
        ppath = pnode->kmer_prg.find_nb_max_path(kmp);
    //std::cout << "found maxpath" << std::endl;
